...
import {researchAdmissionState} from './research-promotions.ts';
...
export function editDraft(state:Workspace,book:Book,questions:Question[],reason:string):Workspace {
 const parsed=parseQuestions(questions,book);
...
export function finalise(state:Workspace,book:Book):Workspace {
 ...
 if(state.draft.some(question=>researchAdmissionState(question,book.id)==='expired'))throw Error('A research-backed question has changed. Restore its verified wording or replace it before finalising.');
 const diagnostics=validateForBook(book,parseQuestions(state.draft,book));if(!diagnostics.valid)throw Error('Resolve the editorial issues before finalising.');
 return {...state,diagnostics,final:{questions:structuredClone(state.draft),date:new Date().toISOString()}};
}
export function readWorkspace(raw:string,book:Book):Workspace {
 ...
 state.draft=parseQuestions(state.draft,book);state.diagnostics=validateForBook(book,state.draft);
 for(const entry of state.history){parseQuestions(entry.questions,book);if(typeof entry.reason!=='string'||typeof entry.time!=='string')throw Error('Invalid history.');}
 for(const entry of state.chat)if(typeof entry.role!=='string'||typeof entry.text!=='string')throw Error('Invalid chat history.');
 if(state.final){state.final.questions=parseQuestions(state.final.questions,book);if(typeof state.final.date!=='string')throw Error('Invalid Chair version.');}
